...
if (searchBtn) {
 ...
 const urlHrf=()=>{
    let urllink=navigator.clipboard.writeText(window.location.href)
    
   alert('url has been copied',urllink)
 }
async function searchMovies() {
    ...
    let res = await fetch(`https://www.omdbapi.com/?apikey=4e2432ef&s=${searchInput.value.trim()}`)
    ...
    movies.forEach(async (movie) => {
        let response = await fetch(`https://www.omdbapi.com/?&apikey=4e2432ef&i=${movie.imdbID}`)
